...
if df_k is not None and len(df_k) > 30:
    ...
    main_fig.add_annotation(x=end_d, y=stop_price, text="🛡️ 防守", showarrow=False, row=1, col=1)
            ...
            with st.spinner(f"正在模擬過去 {backtest_days} 天真實走勢..."):
                ...
                for i in range(max(0, len(df_k) - backtest_days), len(df_k) - max_horizon):
                    ...
                    for _, row in future.iterrows():
                        ...
                        low = float(row['最低價'])
                        if is_bull:
                            if low <= stop_p: hit_stop = True; break
                            if high >= add_on_p: hit_addon = True; break
                        else:
                            if high >= stop_p: hit_stop = True; break
                            if low <= add_on_p: hit_addon = True; break
    ...
    with tab3:
        ...
        if st.button("產生資金曲線", type="primary"):
            np.random.seed(42)
            wins = np.random.rand(sim_trades) < 0.52
            pnls = np.where(wins, risk_dist * 1.45, -risk_dist)
    ...
    st.info("👈 請在上方側邊欄上傳 K 線與選擇權報價檔案後開始使用")
